...
setInterval(
	(function a() {
		...
		document.querySelector<HTMLDivElement>(
			".time-widget"
		).innerHTML = `<svg class="time__icon" height="24" viewBox="0 0 24 24" width="24"><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67z"></path></svg>${new Date().toLocaleString(
			"en-UK",
			{
				hour: "numeric",
				minute: "numeric"
...
const elem = document.querySelector<HTMLDivElement>(".terminal");
...
function onMouseDown(g: MouseEvent) {
	window.event;
	g.preventDefault();
	pos3 = g.clientX;
	pos4 = g.clientY;
...
function onMouseMuse(h: MouseEvent) {
	window.event;
	h.preventDefault();
	pos1 = pos3 - h.clientX;
	pos2 = pos4 - h.clientY;
...
function touchHandler(i: TouchEvent) {
